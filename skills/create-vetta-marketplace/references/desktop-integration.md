...
- branch: the branch containing `.vetta/marketplace.json`, normally `main`
...
The upstream OpenVetta distribution uses `openvetta/vetta-official-marketplace` by default. Distribution owned built in sources are always present and enabled; an ordinary user owned marketplace should be added as a custom source instead.
...
```powershell
$env:VETTA_MARKETPLACE_CANDIDATE_ROOT = 'C:\path\to\marketplace'
...
The candidate test assembles the Git tracked marketplace snapshot and serves local `.release-artifacts/` bytes through mocked network requests. It uses a temporary Vetta home and does not publish a repository or Release.
